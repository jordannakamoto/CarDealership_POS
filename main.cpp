...
bool MyApp::OnInit() {
    ...
    MyFrame *frame = new MyFrame("Car Dealership App", wxDefaultPosition, wxSize(940, 450));
    frame->Show(true);
...
void MyFrame::OnMakeSale(wxCommandEvent& event) {
    ...
    salesFormSizer3->Add(postButton, 0, wxALIGN_RIGHT | wxALL, 5);
    salesFormSizer3->Add(cancelButton, 0, wxALIGN_RIGHT | wxALL, 5);
    ...
    if(salesFormPanel != NULL){
       ...
       if (makeModelYearDropdown != nullptr) {
        ...
        if (customerNameTextCtrl != nullptr) {
            cout << customerNameTextCtrl->GetValue() << endl;
        }
        if (phoneTextCtrl != nullptr) {
            cout << phoneTextCtrl->GetValue() << endl;
        }
        if (emailTextCtrl != nullptr) {
            cout << emailTextCtrl->GetValue() << endl;
        }
        if (addressTextCtrl != nullptr) {
            cout << addressTextCtrl->GetValue() << endl;
        }
        if (priceTextCtrl != nullptr) {
            cout << priceTextCtrl->GetValue() << endl;
        }

        // CREATE new customer
        // CREATE new request from db
        // CreateSaleRow(session, , , ,vin, price, todays date)
